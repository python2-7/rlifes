...
#     'from': 'World Trader',
#     'to': '+84962657798',
#     'text': 'Your World Trader verification code is '
# })
# print(testq)
...
def get_totp_uri(otp_secret, user):
  return 'otpauth://totp/Caribe:{0}?secret={1}&issuer=World Trade' \
...
def update_password():
    ...
    if request.method == 'POST':
        ...
        if old_password == "" or new_password == "":
            flash({'msg':'Mật khẩu cũ không hợp lệ. Vui lòng thử lại!', 'type':'danger'})
            return redirect('/user/setting')
        ...
        if new_password == repeat_new_password and check_password(user.password, old_password) == True:
            flash({'msg':'Cập nhật mật khẩu thành công', 'type':'success'})
    ...
    if int(positon) == 1 or int(positon) == 2:
        if int(positon) == 1:
            if binary.left == '':
                ...
                flash({'msg':'Vị trí không hợp lệ. Vui lòng thử lại!', 'type':'danger'})
                return redirect('/account/dashboard')
        if int(positon) == 2:
            if binary.right == '':
                return render_template('account/signup_in_tree.html', data=value)
...
def registersubmit_intree():
    if request.method == 'POST':
        ...
        sponser = db.User.find_one({'customer_id': request.form['p_node']})
        if sponser is None:
            return redirect('/account/dashboard')
        ...
        if int(request.form['position']) == 1:
            if binary.left != '':
                flash({'msg':'Vị trí đã tồn tại. Xin vui lòng thử lại!', 'type':'danger'})
        ...
        if request.form['fullname'] == '' or request.form['cmnd'] == '' or request.form['bankname'] == '' or request.form['telephone'] == '' or request.form['p_node'] == '' or request.form['email'] == '' or request.form['birthday'] == '' or request.form['account_number'] == '' or request.form['address'] == '' or request.form['brandname'] == '' or request.form['password'] == '' or request.form['account_horder'] == '':
            flash({'msg':'Vui lòng nhập đầy đủ thông tin hệ thống yêu cầu!', 'type':'danger'})
            return redirect('/user/new/'+str(request.form['position'])+'/'+str(request.form['p_binary'])+'/'+str(request.form['p_node'])+'')
        else:
