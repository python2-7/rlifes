...
@admin1_ctrl.route('/thu-nhap-f1', methods=['GET', 'POST'])
def ProfitDaiylythunhapf1():
    error = None
    if session.get('logged_in_admin') is None:
        return redirect('/admin/login')
        
    query = db.historys.find({'type': {'$regex': 'thunhapF1'}})
    data ={
            'menu' : 'thu-nhap-f1',
            'history': query
        }
...
def ProfitDaiylycancap():
    ...
    query = db.historys.find({'type': {'$regex': 'hoahongcannhanh'}})
    data ={
            'menu' : 'cancap',
            'history': query
        }
    return render_template('admin/profit-cancap.html', data=data)
...
def ProfitDaiylysystem():
    ...
    query = db.historys.find({'type': {'$regex': 'thunhaptrenthunhap'}})
    
    data ={
            'menu' : 'profit-system',
            'query': query,
        }
    return render_template('admin/profit-system-history.html', data=data)

@admin1_ctrl.route('/thong-ke', methods=['GET', 'POST'])
def Thongkesssystem():
    error = None
    if session.get('logged_in_admin') is None:
        return redirect('/admin/login')
    ...
    return render_template('admin/thong-ke.html', data=data)
@admin1_ctrl.route('/profit-system-history', methods=['GET', 'POST'])
def ProfitDaiylysystemhistory():
    error = None
    if session.get('logged_in_admin') is None:
        return redirect('/admin/login')

    
    query = db.historys.find({'type': {'$regex': 'chuongtrinhthuong'}})
    data ={
            'menu' : 'profit-system',
...
@admin1_ctrl.route('/commision-system', methods=['GET', 'POST'])
def CommisionSystemhistory():
    error = None
    if session.get('logged_in_admin') is None:
        return redirect('/admin/login')

    
    query = db.historys.find({'type': {'$regex': 'system'}})
    data ={
            'menu' : 'ommision-system',
            'query' : query
       
        }
    return render_template('admin/commision-system-history.html', data=data)
